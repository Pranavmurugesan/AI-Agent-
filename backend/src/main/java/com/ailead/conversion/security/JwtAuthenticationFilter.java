...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
                ...
                Optional<User> userOptional = userRepository.findByIdWithOrganization(userId);
                if (userOptional.isPresent() && userOptional.get().isActive() && userOptional.get().getOrganization().isActive()) {
    ...
    private String getJwtFromRequest(HttpServletRequest request) {
        // 1. Check HttpOnly Cookie first (Primary Secure Web Method)
